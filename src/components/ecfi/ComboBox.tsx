    ...
    <div ref={ref} className="relative flex-1">
      <div className="flex gap-1 items-center">
        <input
          ...
          className="w-full px-2 py-1.5 border border-[var(--card-border)] bg-[var(--bg-main)] text-[var(--text-main)] text-[13px] font-mono focus:outline-none focus:border-[var(--primary-blue)] focus:ring-[3px] focus:ring-[var(--primary-blue-soft)] rounded-lg"
        />
        {isNew && onSaveNew && (
          <button
            onClick={() => onSaveNew(value.trim())}
            title="Save to catalog"
            className="bg-[var(--success)]/15 text-[var(--success)] border border-[var(--success)]/30 px-2 py-1 cursor-pointer text-[11px] whitespace-nowrap font-semibold hover:bg-[var(--success)]/25 transition-colors rounded-lg"
          >
            + Save
          </button>
        )}
        ...
        <div ref={listRef} className="absolute top-full left-0 right-0 z-[999] bg-[var(--card-bg)] border border-[var(--card-border)] max-h-[220px] overflow-y-auto shadow-xl rounded-lg">
          {filtered.map((item, idx) => (
            <div
              ...
              className={`px-2.5 py-[7px] cursor-pointer text-[12px] border-b border-[var(--card-border)] font-mono transition-colors ${
                idx === highlightIndex
                  ? "bg-[var(--primary-blue-soft)] text-[var(--primary-blue)]"
                  : "text-[var(--text-secondary)] hover:bg-[var(--primary-blue-soft)] hover:text-[var(--primary-blue)]"
              }`}
            >
              {item.description}
              <span className="ml-2 text-[10px] text-[var(--text-muted)]">{item.default_unit}</span>
            </div>
          ))}
        </div>
      )}
