...
export function VolumeBreakdown({ ftgLines, slabLines }: VolumeBreakdownProps) {
  ...
  const renderLines = (lines: LineItem[]) =>
    lines
      ...
      .map((l, i) => {
        ...
        return (
          <div key={i} className="py-1.5 border-b border-[var(--card-border)]">
            <div className="flex justify-between items-center gap-3">
              <div className="flex-1 min-w-0">
                <div className="text-[12px] text-[var(--text-main)] overflow-hidden text-ellipsis whitespace-nowrap">{l.description}</div>
                <div className="text-[10px] text-[var(--text-muted)] mt-0.5">
                  {l.qty} {l.unit} {vc.method ? `• ${vc.method}` : ""}
                </div>
              </div>
              <div className={`text-[13px] font-semibold whitespace-nowrap ${isOvr ? "text-ecfi-override-orange-text" : "text-ecfi-vol-blue-text"}`}>
  ...
  return (
    <div className="border-t border-[var(--card-border)] p-4 bg-[var(--section-bg)]">
      {/* Summary bar */}
      <div className="flex gap-4 mb-4 p-3 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-lg">
        {[
          ...
          <div key={i} className={`flex-1 text-center ${i < 3 ? "border-r border-[var(--card-border)] pr-4" : ""}`}>
            <div className={`text-[9px] ${item.colorClass} tracking-widest uppercase font-semibold mb-1`}>{item.label}</div>
      ...
      <div className="grid grid-cols-2 gap-5">
        <div>
          <div className="flex justify-between items-center mb-2.5">
            <span className="text-[11px] font-semibold text-[var(--text-secondary)] tracking-widest uppercase">Footings & Walls Section</span>
            <span className="text-sm font-semibold text-ecfi-vol-blue-text">{ftgYards.toFixed(2)} CY</span>
          </div>
          {ftgLines.filter((l) => l.description && l.qty).length === 0 && (
            <div className="text-[11px] text-[var(--text-muted)] italic py-2">No items entered</div>
          )}
          {renderLines(ftgLines)}
          {ftgLines.filter((l) => l.description && l.qty).length > 0 && (
            <div className="flex justify-end gap-3 text-[11px] font-semibold py-2 border-t border-[var(--card-border)] mt-1">
        ...
        <div>
          <div className="flex justify-between items-center mb-2.5">
            <span className="text-[11px] font-semibold text-[var(--text-secondary)] tracking-widest uppercase">Slabs Section</span>
            <span className="text-sm font-semibold text-ecfi-vol-blue-text">{slabYards.toFixed(2)} CY</span>
          </div>
