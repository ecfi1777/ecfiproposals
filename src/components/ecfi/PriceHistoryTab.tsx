...
export function PriceHistoryTab({ onLoadProposal }: PriceHistoryTabProps) {
  ...
  if (loading) {
    return <div className="text-center py-12 text-muted-foreground animate-pulse">Loading price history…</div>;
  }
  ...
  if (records.length === 0) {
    return (
      <div className="text-center py-16 space-y-3">
        ...
        <div className="text-lg font-bold text-muted-foreground">No pricing data yet</div>
        <div className="text-sm text-muted-foreground">Save proposals to start building your price history.</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {/* Filters */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 p-4 border border-ecfi-panel-border bg-ecfi-panel-bg">
        <div className="relative lg:col-span-1">
          ...
          <Input
            ...
            className="pl-8 h-9 text-xs font-mono bg-background border-ecfi-panel-border"
          />
        </div>
        <Select value={builderFilter} onValueChange={setBuilderFilter}>
          <SelectTrigger className="h-9 text-xs font-mono bg-background border-ecfi-panel-border">
        ...
        <div className="relative">
          <Calendar className="absolute left-2.5 top-2.5 w-3.5 h-3.5 text-muted-foreground pointer-events-none" />
          <Input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="pl-8 h-9 text-xs font-mono bg-background border-ecfi-panel-border" placeholder="From" />
        </div>
        <div className="relative">
          <Calendar className="absolute left-2.5 top-2.5 w-3.5 h-3.5 text-muted-foreground pointer-events-none" />
          <Input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="pl-8 h-9 text-xs font-mono bg-background border-ecfi-panel-border" placeholder="To" />
        </div>
      </div>

      <div className="text-xs text-muted-foreground">{summaries.length} items · {filtered.length} records</div>
      ...
      <div className="space-y-2">
        {summaries.map(item => {
          ...
          return (
            <div key={item.description} className="border border-ecfi-panel-border bg-ecfi-panel-bg">
              <button
                onClick={() => setExpandedItem(isOpen ? null : item.description)}
                className="w-full flex items-center justify-between p-4 text-left hover:bg-muted/30 transition-colors"
              >
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-sm truncate">{item.description}</div>
                ...
                <div className="flex items-center gap-6 text-xs shrink-0 ml-4">
                  ...
                  <div className="text-center">
                    ...
                    <div className="font-bold">{fmtCurrency(item.minPrice)} – {fmtCurrency(item.maxPrice)}</div>
                  </div>
                  <div className="text-center">
              ...
              {isOpen && (
                <div className="border-t border-ecfi-panel-border overflow-x-auto">
                  <table className="w-full text-xs font-mono">
                    <thead>
                      <tr className="text-[10px] text-muted-foreground uppercase tracking-wider border-b border-ecfi-panel-border">
                    ...
                    <tbody>
                      {item.records.map(r => (
                        <tr key={r.id} className="border-b border-ecfi-panel-border/50 hover:bg-muted/20 transition-colors">
                          <td className="p-2 pl-4">{new Date(r.recorded_at).toLocaleDateString()}</td>
                          <td className="p-2">{r.builder || "—"}</td>
                          <td className="p-2">{r.job_location || "—"}</td>
                          ...
                          <td className="p-2">
                            {r.proposal_id && (
                              <button
                                ...
                                className="text-ecfi-vol-blue-text hover:underline flex items-center gap-1"
                                title="Open proposal"
                              >
